...
def GetCategoriesForMain():
    ...
    try:
        ...
        try:
            ...
            EnteredRole = 0
        FinalCategoriesList = []
        FoundCategories = Event_Categories.query.all()
        ...
        for CategoryItem in FoundCategories:
            FinalCategoriesList.append(
                {"category_id": CategoryItem.category_id, "category_name": CategoryItem.category_name,
    ...
    try:
        ...
        if IsNearYou == True:
            ...
            FoundEvents = db.session.query(Event, Event_Days, Event_Categories_Details).filter(
                ...
                Event_Categories_Details.category_id == CategoryID).all()
        FoundEventsCount = 0
        FoundEventsModifyed = []
...
def GetEventDetails():
    ...
    try:
        ...
        FoundCategories = db.session.query(Event_Categories_Details, Event_Categories).filter(
            Event_Categories_Details.event_id == EventID,
            Event_Categories_Details.category_id == Event_Categories.category_id).all()
        ...
        for CategoryItem in FoundCategories:
            CategoriesList.append(
                {"ID": CategoryItem.Event_Categories.category_id, "Name": CategoryItem.Event_Categories.category_name})
...
def GetFilteredEventsWithPagination():
    ...
    try:
        ...
        if CategoryID == 0 and RegionID == 0:
            ...
            FoundEvents = db.session.query(Event, Event_Days, Event_Categories_Details).filter(
                Event.event_id == Event_Days.event_id, Event.event_id == Event_Categories_Details.event_id,
                Event_Categories_Details.category_id == CategoryID, Event.region_id == RegionID).all()
        FoundEventsCount = 0
        FoundEventsModifyed = []
        for EventItem in FoundEvents:
